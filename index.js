...
const main = async () => {
    ...
    const usdt = new ethers.Contract('0xdAC17F958D2ee523a2206206994597C13D831ec7', erc20Abi, provider)
    ...
    const batchCalldata2 = usdcHolders.map(address => {
        return {
            target: usdt.address,
            callData: usdt.interface.encodeFunctionData("balanceOf", [address])
        }
    })
    ...
    outputF.map(({ success, returnData }, i) => {
        const address = usdcHolders[i]
        ...
        const amount = usdt.interface.decodeFunctionResult("balanceOf", returnData)[0]
        console.log(`usdt.balanceOf(${address}) => ${ethers.utils.formatUnits(amount, 6)}`)
        return [address, amount]
    })
}
